function(download_openrct2_zip)
    ...
    message(STATUS "Starting download for ${DOWNLOAD_OPENRCT2_ZIP_URL}")
    if (NOT EXISTS ${DOWNLOAD_OPENRCT2_DOWNLOAD_DIR})
        message(STATUS "Download dir ${DOWNLOAD_OPENRCT2_DOWNLOAD_DIR} not found. Downloading zip")
        set(DOWNLOAD_ZIP 1)
    else ()
        if (EXISTS "${DOWNLOAD_OPENRCT2_DOWNLOAD_DIR}/${ZIP_FILE_NAME}.zipversion")
            ...
            message(STATUS "Cache ${DOWNLOAD_OPENRCT2_DOWNLOAD_DIR} up to date. Skipping download")
        else ()
            message(STATUS "${DOWNLOAD_OPENRCT2_DOWNLOAD_DIR}/${ZIP_FILE_NAME}.zipversion missing. Downloading")
            set(DOWNLOAD_ZIP 1)
        endif ()
    endif ()
    message(STATUS "Download mid ${DOWNLOAD_ZIP} for ${DOWNLOAD_OPENRCT2_ZIP_URL}")

    foreach(check_exist_file_or_dir ${DOWNLOAD_OPENRCT2_SKIP_IF_EXISTS})
        message(STATUS "Checking existence of ${check_exist_file_or_dir}...")
        if (EXISTS ${check_exist_file_or_dir})
            message(STATUS "${check_exist_file_or_dir} exists, skipping download")
            set(DOWNLOAD_ZIP 0)
            break()
        else ()
            message(STATUS "${check_exist_file_or_dir} not found")
        endif ()
    endforeach(check_exist_file_or_dir)
    message(STATUS "Download caches checked: ${DOWNLOAD_ZIP} for ${DOWNLOAD_OPENRCT2_ZIP_URL}")

    if (DOWNLOAD_ZIP)
        message(STATUS "Downloading ${DOWNLOAD_OPENRCT2_ZIP_URL}")
        file(DOWNLOAD
    ...
    message(STATUS "Download of ${DOWNLOAD_OPENRCT2_ZIP_URL} done")
