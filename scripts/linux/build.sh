...
pushd build
	...
	then
		...
		docker run -v "$PARENT":"$PARENT" -w "$PARENT"/build -i -t janisozaur/openrct2:64bit-only bash -c "cmake ../ -DWITH_TESTS=on $OPENRCT2_CMAKE_OPTS && ninja $OPENRCT_MAKE_OPTS && ctest --output-on-failure"
