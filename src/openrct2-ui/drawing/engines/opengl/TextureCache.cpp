...
{
    CachedTextureInfo *cachedInfo = _imageTextureMap[image];
    if (cachedInfo != nullptr && cachedInfo->index != 0xFFFFFFFF)
    {
        _atlases[cachedInfo->index].Free(*cachedInfo);
        cachedInfo->index = 0xFFFFFFFF;
    }
    /*
...
const CachedTextureInfo* TextureCache::GetOrLoadImageTexture(uint32 image)
{
    ...
    */
    ...
    {
        auto cacheInfo = LoadImageTexture(image);
        cachedInfo = new CachedTextureInfo(cacheInfo);
        _imageTextureMap[image] = cachedInfo;
    }
    else if (cachedInfo->index == 0xFFFFFFFF)
    {
        *_imageTextureMap[image] = LoadImageTexture(image);
    }
    return cachedInfo;
}
...
void TextureCache::FreeTextures()
{
    // Free array texture
    glDeleteTextures(1, &_atlasesTexture);
    std::fill(_imageTextureMap.begin(), _imageTextureMap.end(), nullptr);
}
