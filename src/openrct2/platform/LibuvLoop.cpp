...
namespace OpenRCT2::Platform
{
    ...
    static void on_read(uv_fs_t* req)
    {
        ...
        if (result < 0)
        {
            data->callback({}, static_cast<int>(result));
    ...
    static void on_stat(uv_fs_t* stat_req)
    {
        ...
        int64_t result = stat_req->result;
    ...
    static void on_open(uv_fs_t* req)
    {
        auto* data = static_cast<AsyncReadData*>(req->data);
        int64_t result = req->result;
        // Don't cleanup open_req yet, we need its result (fd) for subsequent operations
        if (result < 0)
        {
            data->callback({}, static_cast<int>(result));
            uv_fs_req_cleanup(&data->open_req);
        ...
        uv_fs_fstat(LibuvLoop::Get().GetLoop(), &data->stat_req, static_cast<uv_file>(result), on_stat);
    }
