...
const rct_xy_element offsets[4] =
{ 
    { -32,   0 },
    {   0,  32 },
    {  32,   0 },
...
void virtual_floor_paint(paint_session * session)
{
    ...
    do
    {
        ...
        {
            if (virtualFloorClipHeight < mapElement->clearance_height)
            {
                weAreBelowGround    = true;
            }
            else if (virtualFloorClipHeight < mapElement->base_height + 2 &&
        ...
        if (virtualFloorClipHeight >= mapElement->clearance_height ||
            virtualFloorClipHeight < mapElement->base_height)
        {
            continue;
        }

        if (elementType == MAP_ELEMENT_TYPE_WALL ||
            elementType == MAP_ELEMENT_TYPE_BANNER)
        {
