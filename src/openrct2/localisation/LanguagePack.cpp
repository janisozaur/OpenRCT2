    ...
    class LanguagePack final : public ILanguagePack
    {
        ...
        class LanguagePack final : public ILanguagePack
        {
            ...
            private:
                ...
                // Try platform-specific loading first (handles embedded assets on mobile platforms)
                std::vector<uint8_t> platformData;
                if (Platform::TryLoadFile(path, platformData))
                {
                    fileData.assign(platformData.begin(), platformData.end());
                    fileData.push_back('\0'); // Ensure null termination
                    LOG_INFO(
                        "Successfully loaded language from platform-specific source: %s (%zu bytes)", path,
                        platformData.size());
                    return FromText(id, fileData.data());
                }

                // Fall back to filesystem loading
                LOG_VERBOSE("Using filesystem for language file: %s", path);
                try
                {
                    ...
                    {
                        std::string assetPath = "openrct2/"
                            + pathStr.substr(sdcardPos + 17); // 17 = length of "/sdcard/openrct2/"
                        LOGD("Trying to load language from embedded asset: %s", assetPath.c_str());

                        auto& assetManager = AndroidAssetManager::getInstance();
                        if (assetManager.assetExists(assetPath))
                        {
                            auto data = assetManager.readAsset(assetPath);
                            if (!data.empty())
                            {
                                fileData.assign(data.begin(), data.end());
                                fileData.push_back('\0'); // Ensure null termination
                                LOGI(
                                    "Successfully loaded language from embedded asset: %s (%zu bytes)", assetPath.c_str(),
                                    data.size());
                                return FromText(id, fileData.data());
                            }
                            else
                            {
                                LOGE("Failed to read data from embedded asset: %s", assetPath.c_str());
                            }
                        }
                        else
                        {
                            LOGD("Asset not found in embedded assets: %s", assetPath.c_str());
                        }
                    }

                    // Fall back to filesystem if asset loading failed
                    LOGD("Falling back to filesystem for language file: %s", path);
#endif
                ...
                private:
                    ...
                    // Parsing
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    // Partial support to open an uncompiled language file which parses tokens and converts them to the
                    // corresponding character code. Due to resource strings (strings in scenarios and objects) being written to
                    // the original game's string table, get_string will use those if the same entry in the loaded language is
                    // empty.
                    //
                    // Unsure at how the original game decides which entries to write resource strings to, but this could affect
                    // adding new strings for the time being. Further investigation is required.
                    //
                    // When reading the language files, the STR_XXXX part is read and XXXX becomes the string id number.
                    // Everything after the colon and before the new line will be saved as the string. Tokens are written with
                    // inside curly braces {TOKEN}. Use # at the beginning of a line to leave a comment.
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
