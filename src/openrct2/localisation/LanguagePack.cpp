...
namespace OpenRCT2
{
    ...
    class LanguagePack final : public ILanguagePack
    {
        ...
        private:
...
#ifdef __ANDROID__
            // Initialize AndroidClassLoader if needed before trying to load language files
            ...
            if (!classLoaderInitialized)
            {
                try
                {
                    ...
                    LOGI("AndroidClassLoader initialized for language loading");
                }
                catch (const std::exception& e)
                {
                    LOGE("Failed to initialize AndroidClassLoader: %s", e.what());
            ...
            if (sdcardPos != std::string::npos)
            {
                ...
                if (assetManager.assetExists(assetPath))
                {
                    ...
                    if (!data.empty())
                    {
                        ...
                        LOGI(
                            "Successfully loaded language from embedded asset: %s (%zu bytes)", assetPath.c_str(), data.size());
        ...
        private:
            ...
            //
            // Unsure at how the original game decides which entries to write resource strings to, but this could affect adding
            // new strings for the time being. Further investigation is required.
            //
            // When reading the language files, the STR_XXXX part is read and XXXX becomes the string id number. Everything
            // after the colon and before the new line will be saved as the string. Tokens are written with inside curly braces
            // {TOKEN}. Use # at the beginning of a line to leave a comment.
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
