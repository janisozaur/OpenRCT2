...
class NetworkUserManager : public INetworkUserManager
{
private:
    std::map<std::string, NetworkUser *> _usersByHash;

public:
    virtual ~NetworkUserManager()
    {
        DisposeUsers();
    }
    ...
    void UnsetUsersOfGroup(uint8 groupId) override
    {
        for (const auto &kvp : _usersByHash)
        {
    ...
    void RemoveUser(const std::string &hash) override
    {
    ...
    NetworkUser * GetUserByHash(const std::string &hash) override
    {
    ...
    const NetworkUser * GetUserByHash(const std::string &hash) const override
    {
        auto it = _usersByHash.find(hash);
        if (it != _usersByHash.end())
        {
    ...
    const NetworkUser * GetUserByName(const std::string &name) const override
    {
    ...
    NetworkUser * GetOrAddUser(const std::string &hash) override
    {
        NetworkUser * networkUser = GetUserByHash(hash);
        if (networkUser == nullptr)
        {
